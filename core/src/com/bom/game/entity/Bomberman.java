...
public class Bomberman extends EntityBase implements Disposable {
	...
	public void handleInput(float delta) {
		if (!canDestroy) {
			if (Gdx.input.isKeyPressed(Input.Keys.W)) {
				...
				// this.body.applyLinearImpulse(new Vector2(0, 0.05f),
				...
				// this.body.applyLinearImpulse(new Vector2(-0.05f, 0),
				// this.body.getWorldCenter(), true);
			} else if (Gdx.input.isKeyPressed(Input.Keys.D)) {
				animationHandle
						.setCurrentAnimation(State.WALK_RIGHT.getValue());
				this.body.setLinearVelocity(new Vector2(speed, 0));
	...
	private void checkExplode(float deltaTime) {
		...
		if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE) && canPlaceBombs
			...
			bomb = new Bomb();
			bomb.init(this, UnitHelper.coordBox2DSnapToGrid(body.getPosition()),
					flameLength);
			bombs.add(bomb);
			bombCount--;
			canPlaceBombs = false;
	...
	public void definePlayer(Vector2 position) {
		...
		fDef.filter.maskBits = BitCollision.orOperation(BitCollision.WALL,
				...
				BitCollision.ENEMY, BitCollision.ITEM);
		fDef.shape = shape;
	...
	private enum State {
		...
		IDLE_UP("idle_up"), IDLE_DOWN("idle_down"), IDLE_LEFT(
				"idle_left"), IDLE_RIGHT("idle_right"), WALK_LEFT(
						"walk_left"), WALK_RIGHT("walk_right"), WALK_UP(
								"walk_up"), WALK_DOWN(
										"walk_down"), DEAD("dead");
