...
public class Bomb extends EntityBase implements Poolable, Disposable {
	...
	private void defineBomb(Vector2 position) {
		...
		fDef.filter.maskBits = BitCollision.orOperation(BitCollision.BOMBERMAN,
				BitCollision.WALL, BitCollision.BRICK, BitCollision.FLAME,
				BitCollision.ENEMY, BitCollision.BOMB);
	...
	private void explode() {
		...
		Flame flameMid = new Flame(this,
				UnitHelper.coordBox2DSnapToGrid(body.getPosition()),
				Flame.State.FLAME_UP);
		for (Flame.State direction : Flame.State.values()) {
			Vector2 position, nextPosition;
			for (int i = 0; i <= flameLength; i++) {
				position = UnitHelper.coordBox2DSnapToGrid(body.getPosition()
						.add(Flame.State.getOffSet(direction).scl(i)));
				Vector2 temp = body.getPosition()
						.add(Flame.State.getOffSet(direction).scl(i + 1));
	...
	public void update(float delta) {
		...
		if (countDown > 0) {
			sprite.setBounds(
					...
					UnitHelper.pixelsToMeters(animationHandle.getCurrentFrame()
							.getRegionHeight()));
		}
		...
		// System.err.println(this.body.getPosition().x + " " +
