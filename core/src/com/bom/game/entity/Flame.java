...
public class Flame extends EntityBase implements Disposable {
	...
	public Flame(Bomb bomb, Vector2 position, State direction) {
		...
		animationHandle.addAnimation(State.FLAME_UP.getValue(),
				new Animation<TextureRegion>(FRAME_TIME,
						atlas.findRegions(State.FLAME_UP.getValue())));
	...
	private void defineFlame(Vector2 position) {
		...
		fDef.filter.maskBits = BitCollision.orOperation(BitCollision.BOMBERMAN,
				BitCollision.WALL, BitCollision.BRICK, BitCollision.BOMB,
				BitCollision.ENEMY);
	...
	public void update(float delta) {
		sprite.setBounds(
				...
				UnitHelper.pixelsToMeters(16), UnitHelper.pixelsToMeters(16));
	...
	public static enum State {
		FLAME_UP("flame_y"), FLAME_DOWN("flame_y"), FLAME_LEFT(
				"flame_x"), FLAME_RIGHT("flame_x");
		String stateName;
